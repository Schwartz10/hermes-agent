...
Options:
  ...
  --latest               Also tag and push IMAGE_REPO:latest. Requires --push.
  --no-smoke             Skip the container smoke tests.
  --image-repo REPO      Image repository. Default: ghcr.io/williamos-hq/hermes-agent
  --tag TAG              Image tag. Default: williamos-<origin-main-sha>-<head-sha>
...
if [ "$LATEST" -eq 1 ] && [ "$PUSH" -ne 1 ]; then
  die "--latest requires --push"
fi
...
if [ -z "$TAG" ]; then
  TAG="williamos-${MAIN_SHA}-${HEAD_SHA}"
fi
...
if [ "$PUSH" -eq 1 ]; then
  ...
  if [ "$LATEST" -eq 1 ]; then
    echo "Promoting $IMAGE to $LATEST_IMAGE"
